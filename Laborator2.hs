...
myreplicate2 :: Integer -> x -> [x]
myreplicate2 0 x = []
myreplicate2 y x = x : myreplicate2 (y-1) x

myreplicate3 :: Integer -> x -> [x]
myreplicate3 n x 
                | n == 0 = []
                | otherwise = x : myreplicate3 (n-1) x

sumaImp :: [Integer] -> Integer
sumaImp [] = 0
sumaImp (x:xs) = if even x then
                    sumaImp xs
                 else
                    x + sumaImp xs
...
totalLen2 [] = 0
totalLen2 (x:xs) = if head x == 'A' then
                    length x + totalLen2 xs
                   else
                    totalLen2 xs